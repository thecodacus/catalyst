...
const PROVIDER_MODELS = {
  openai: [
    {
      ...
      name: 'GPT-4 Turbo',
      description: 'Most capable, latest GPT-4',
    },
  ...
  openrouter: [
    ...
    },
    {
      ...
      name: 'Llama 2 70B',
      description: 'Open source via OpenRouter',
    },
    {
      id: 'mistralai/mixtral-8x7b-instruct',
      name: 'Mixtral 8x7B',
      description: 'MoE model via OpenRouter',
    },
  ],
  gemini: [
    {
  ...
  return withAuth(request, async (_req, authUser) => {
    try {
      ...
      const availableModels =
        PROVIDER_MODELS[
          userSettings.provider as keyof typeof PROVIDER_MODELS
        ] || [];

      // Get list of configured providers
      const configuredProviders = user.settings.providers
        ? Object.keys(user.settings.providers).filter((providerId) => {
